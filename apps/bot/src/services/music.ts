...
import { clamp, formatDuration, type MusicLoopMode } from "@manok/shared";
...
function queueFor(guildId: string, volume: number): Queue {
  const existing = queues.get(guildId);
  if (existing) return existing;
  const queue = { tracks: [], loop: "off" as MusicLoopMode, volume, paused: false };
  queues.set(guildId, queue);
  return queue;
}

function spotifyToSearch(query: string) {
  if (!/open\.spotify\.com\/(track|album|playlist)/i.test(query)) return query;
  return `ytsearch:${query} official audio`;
}

async function ensureVoice(interaction: ChatInputCommandInteraction) {
  const member = interaction.member as GuildMember | null;
  const channel = member?.voice.channel;
  if (!channel) throw new Error("Join a voice channel first and MANOK will auto-join you.");
  return channel;
}

export async function handleMusicCommand(interaction: ChatInputCommandInteraction, subcommand: string) {
  ...
  try {
    ...
    if (subcommand === "loop") {
      queue.loop = interaction.options.getString("mode", true) as MusicLoopMode;
