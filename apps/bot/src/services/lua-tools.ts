...
function keyMaterial(salt: Buffer) {
  return crypto.pbkdf2Sync(env.ENCRYPTION_MASTER_KEY, salt, iterations, 32, "sha512");
}

async function readAttachment(interaction: ChatInputCommandInteraction) {
  const file = interaction.options.getAttachment("file", true);
  if (file.size > luaCryptoLimits.maxBytes) throw new Error(`File exceeds ${luaCryptoLimits.maxBytes / 1024} KB limit.`);
  const extension = path.extname(file.name).toLowerCase();
  if (!luaCryptoLimits.extensionAllowList.includes(extension as ".lua")) {
...
async function staffLog(interaction: ChatInputCommandInteraction, action: "lua_encrypt" | "lua_decrypt", metadata: Record<string, unknown>) {
  if (!interaction.guildId) return;
  await writeAuditLog({ guildId: interaction.guildId, userId: interaction.user.id, action, metadata });
  if (!env.STAFF_LOG_WEBHOOK_URL) return;
...
export async function handleLuaCommand(interaction: ChatInputCommandInteraction, subcommand: string) {
  try {
    if (subcommand === "hash") {
    ...
    if (subcommand === "decrypt") {
      ...
      if (config.ownerOnlyDecrypt && !isOwner) throw new Error("Owner-only decrypt mode is enabled for this guild.");
      if (!ownsFile && !isOwner && !hasStaffRole) throw new Error("You can only decrypt MANOK files that you own.");
    ...
    if (interaction.deferred) await interaction.editReply({ embeds: [premiumEmbed("Lua Tool Blocked", message)] });
    else await interaction.reply({ embeds: [premiumEmbed("Lua Tool Blocked", message)], ephemeral: true });
  }
}
