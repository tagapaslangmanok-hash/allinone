...
function allCommandsEmbed() {
    return new EmbedBuilder()
        ...
        .addFields({ name: "Security", value: categories.security.commands.map((c) => `\`${c}\``).join(", ") }, { name: "Music", value: categories.music.commands.map((c) => `\`${c}\``).join(", ") }, { name: "Lua Tools", value: categories.lua.commands.map((c) => `\`${c}\``).join(", ") }, { name: "Community", value: ["/manok ticket", "/manok verify", "/manok welcome", "/manok embed", "/manok giveaway", "/manok suggest", "/manok backup", "/manok dashboard"].map((c) => `\`${c}\``).join(", ") })
...
function buttonRow() {
    return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("manok:help:all").setLabel("All Commands").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId("manok:help:report").setLabel("Report").setStyle(ButtonStyle.Danger), new ButtonBuilder().setLabel("Invite").setStyle(ButtonStyle.Link).setURL(inviteUrl), new ButtonBuilder().setCustomId("manok:help:support").setLabel("Support").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("manok:help:vote").setLabel("Vote").setStyle(ButtonStyle.Secondary));
}
function helpPayload(embed, selected) {
    return {
        embeds: [embed],
        components: [selectRow(selected), buttonRow()]
    };
}
export function mainHelpPayload(interaction) {
