import { ActionRowBuilder, AttachmentBuilder, ButtonBuilder, ButtonStyle, ChannelType, Events, PermissionFlagsBits } from "discord.js";
...
export async function handleWelcomeConfig(interaction) {
...
export async function sendLeave(member) {
    const config = await getGuildConfig(member.guild.id);
    if (!config.welcome.enabled || !config.welcome.leaveChannelId)
        return;
...
export async function handleGiveaway(interaction) {
    if (!interaction.guildId || !interaction.channel?.isTextBased() || !canSend(interaction.channel))
        return;
    const prize = interaction.options.getString("prize", true);
    const minutes = interaction.options.getInteger("minutes") ?? 10;
    const winnerCount = interaction.options.getInteger("winners") ?? 1;
    const endsAt = Date.now() + minutes * 60_000;
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("giveaway:join:pending").setLabel("Join Giveaway").setStyle(ButtonStyle.Success));
    ...
    const activeRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`giveaway:join:${message.id}`).setLabel("Join Giveaway").setStyle(ButtonStyle.Success));
...
export function installCommunityEvents(client) {
    botClient = client;
